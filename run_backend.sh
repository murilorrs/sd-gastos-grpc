...
# Só o que o Gateway precisa: segredo do JWT e o usuário de login. (Um laço de
# read em vez de `source <(...)`: o bash 3.2 do macOS lê vazio nesse caso.)
if [ -f .env ]; then
  while IFS= read -r line; do
    export "$line"
  done < <(grep -E '^(JWT_SECRET|GATEWAY_USER|GATEWAY_PASSWORD)=' .env)
fi
...
(cd server && "$PY" cards_service.py) &
(cd server && "$PY" expenses_service.py) &
